...
<div class="container">
    ...
    <form action="{{ route('samples.update', $sample->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select name="patient_id" class="form-control" required>
                @foreach($patients as $patient)
                    <option value="{{ $patient->id }}" {{ $patient->id == $sample->patient_id ? 'selected' : '' }}>{{ $patient->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="date" name="collection_date" class="form-control" value="{{ $sample->collection_date }}" required>
        </div>
        <div class="form-group">
